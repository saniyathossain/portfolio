...
trait CoreLibraryTrait
{
	...
	{
		(string) $parts			= asset($this->backwardToForwardSlash($path));
		...
		if (!File::exists($this->backwardToForwardSlash($path))):
            ...
            // echo (string) '<figure class="progressive"><img data-lity data-lity-target="'.$fullPath.'" data-progressive="'.$fullPath.'" src="'.asset($this->backwardToForwardSlash($path)).'" '.' '.$attributes.'/></figure>';
            echo (string) '<img src="'.asset($this->backwardToForwardSlash($path)).'" '.' '.$attributes.'/>';
		endif;
	}
